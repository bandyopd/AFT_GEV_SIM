functions {
  ...
  // N: sample size
  vector phi0(vector eta, vector u, int N) {
      vector[N] output;
      for (i in 1:N) {
          if (eta[i] < u[2]) {
              output[i] = (u[2]*(eta[i]+1.0)+0.5-0.5*pow(eta[i],2.0))/(u[2]+1);
          }
          else {
  ...
  vector phil(vector eta, vector u, int N, int l){
      vector[N] output;
      for (i in 1:N) {
          if (eta[i] < u[l]) {
  ...
  matrix phiAll(vector eta, vector u, int N, int L){
      matrix[N,L+1] phiX; 
      phiX[,1] = phi0(eta,u,N);
      for (j in 2:L) {
...
data {
  ...
  int<lower=0> L;                             // the number of bases
}
...
transformed data {
  ...
  vector[L+1] u;                             // u: vector of specified knots from -1 to 1
  
  u = linspaced_vector(L+1,-1.0,1.0);
  // u[1] = -1;
  // for(ss in 2:(L+1))
  //   u[ss] = u[ss-1] + 2.0/L;
...
parameters {
  ...
  vector[L+1] alpha;                           // GP basis coefficients; 
...
//   }
// 
//   yHat += phiAll(si_linear_select,u,nobs,L)*abs(alpha) + intercept;
//   yHat_cens += phiAll(si_linear_select_cens,u,ncens,L) * abs(alpha) + intercept;
//   
//   // //making sure the parameter values are such that the GEV support is satisfied; otherwise revise the scale_GEV  
//   // constraint = max(append_row(yHat-yvec, yHat_cens-yvec_censor)*shape_GEV)*exp(-scale_GEV_raw);
//   // if(constraint > 1){
//   //   scale_GEV = log(max(append_row(yHat-yvec, yHat_cens-yvec_censor)*shape_GEV));
//   // } else{
//   //   scale_GEV = scale_GEV_raw;
//   // }
//   
...
//   //   constraint = shape_GPD_group*(mean_GPD_group-max(group_sd_local))*pow(scale_GPD_raw_group,-1);
//   // if(constraint > 1){
