...
for(id in 1:50){
  fit <- readRDS(paste0("../Result/simulation/sim", sim_id, "/res_id",id,"_SelectedVariables_GEVBP.RDS"))
  chain_id <- c(1,2)                 #which chain is used for the posterior summaries
...
u <- seq(-1,1,length=M+1)         # equal distanced grid
...
pdf(paste0("../Result/figure/simulation/sim", sim_id, "/function_estimate_GEV_BP_SelectedVariables.pdf"))
...
p1 + labs(y=bquote(f(u)), x="u", title="GEV-BP") +
...
for(id in 1:50){
  ...
  mse_beta[id] <- mean(apply(res[[id]]$sc[((chain_id_final[id]-1)*(nrow(alpha)/2)+1):(chain_id_final[id]*nrow(alpha)/2),], 1, function(x) sum((x-beta_true_vec)^2))) #for better/robust results, posterior median is used 
}
