...
for(id in 1:50){
  fit <- readRDS(paste0("../Result/simulation/sim",sim_id,"/res_id",id,"_SelectedVariables_NormalGP.RDS"))
  chain_id <- c(1,2)
...
pdf(paste0("../Result/figure/simulation/sim",sim_id,"/function_estimate_Normal_GP_SelectedVariables.pdf"))
...
p1 + labs(y=bquote(f(u)), x="u", title="Normal-GP") +
  theme(plot.title = element_text(hjust = 0.5))
dev.off()

### table for beta estimates
mse_beta <- rep(0, 50)
...
ind_all <- readRDS("../Result/simulation/sim",sim_id,"/ind_selected_all_NormalGP.rds")
